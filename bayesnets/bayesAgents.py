...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():               # Obtained full set of observation variables.
            for obsPos in gameState.getHouseWalls(housePos):
                obsVar = OBS_VAR_TEMPLATE % obsPos
                obsVars.append(obsVar)                           # Populating `obsVars`.
                edges.append((FOOD_HOUSE_VAR, obsVar))
                edges.append((GHOST_HOUSE_VAR, obsVar))


    for i in [X_POS_VAR, Y_POS_VAR]:                               # Populating `edges` with every edge in the Bayes Net.
        for j in [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]:
            edges.append((i,j))
    
    variableDomainsDict[X_POS_VAR] = X_POS_VALS                   # Setting each `variableDomainsDict[var] = values`, where `values` is the set of possible assignments to `var`. These should again be set using the constants defined at the top of this file.
    variableDomainsDict[Y_POS_VAR] = [LEFT_TOP_VAL, BOTH_TOP_VAL,  LEFT_BOTTOM_VAL, BOTH_BOTTOM_VAL]
...
def fillYCPT(bayesNet, gameState):
    ...
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)                                        # Filling the CPT that gives the prior probability over the y position variable.
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    fact = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)               # Calling the variable elimination method to obtain p(FoodHouse | everything else)

    Highestprob = float('-inf')                                                                                         # Initializing mostLikePos = -∞.

    AllPossAssigns = fact.getAllPossibleAssignmentDicts()                                                               # List of all assignmentDicts.

    for assign in AllPossAssigns:                                                                                       # Iterates over all assignmentDicts.

        prob = fact.getProbability(assign)                                                                              # Finds the probability.

        if(prob > Highestprob):                                                                                         # Checks if it has the highest probability so far.

            Highestprob = prob                                                                                          # If yes, stores the probability as the highest probability so far.

            mostLikePos = assign                                                                                        # Stores the assign as most prabable position.

    return mostLikePos                                                                                                  # Returns the most probable position.
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        fact = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)
        ...
        for assign in AllPossAssigns:                                                                                       # Iterates over all assignmentDicts.
            ...
            if (assign[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assign[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL): 
                ...
                leftProb = fact.getProbability(assign)

            elif (assign[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assign[GHOST_HOUSE_VAR] == TOP_LEFT_VAL): 

                rightProb = fact.getProbability(assign)


        leftExpectedValue = leftProb * WON_GAME_REWARD + (1 - leftProb) * GHOST_COLLISION_REWARD                            # Computing the value of entering the left house immediately.

        rightExpectedValue = rightProb * WON_GAME_REWARD + (1 - rightProb) * GHOST_COLLISION_REWARD                         # Computing the value of entering the right house immediately.
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        ProbsAndOutcomes = self.getExplorationProbsAndOutcomes(evidence)                                    # List of pairs of the form (prob, explorationEvidence), where `evidence` is a new evidence dictionary with all of the missing observations filled in, and `prob` is the probability of that set of observations occurring.

        for (prob, explorationEvidence) in ProbsAndOutcomes:                                                # Iterating over each pair.

            HigestExpVal = max(self.computeEnterValues(explorationEvidence, enterEliminationOrder))         # Compute the expected value (expectedValue) of first exploring the remaining unseen house, and then entering the house with highest expected value (HigestExpVal).

            expectedValue = expectedValue + prob * HigestExpVal

        return expectedValue                                                                                # Returning the expected value.
