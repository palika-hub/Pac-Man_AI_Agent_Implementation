...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        facts = bayesNet.getAllCPTsWithEvidence(evidenceDict)                           # A list containing all the Conditional Probability Tables.

        for variable in eliminationOrder:                                               # Iterating over variables according to eliminationOrder.

            facts, join_fact = joinFactorsByVariable(facts, variable)                   # Joining.
 
            if len(join_fact.unconditionedVariables()) <= 1:                            # Checks if factor has only one or no unconditioned variable.

                continue                                                                # If yes, factor not eliminated.

            else:

                elim_fact = eliminate(join_fact, variable)                              # Eliminating.

                facts.append(elim_fact)

        join_fact = joinFactors(facts)

        norm_fact = normalize(join_fact)
